...
contract IMigrationContract {
    function migrate(address addr, uint256 CRAT) returns (bool success);
}
...
contract cyberangeToken is StandardToken, SafeMath {

    // metadata
    string  public constant name = "cyberange Token";
    string  public constant symbol = "CRAT";
    uint256 public constant decimals = 18;
    string  public version = "1.0";

    // contracts
    address public ethFundDeposit;          // deposit address for ETH for cyberange Team.
    address public newContractAddr;         // the new contract for cyberange token updates;
    ...
    uint256 public tokenExchangeRate = 625;             // 625 CRAT tokens per 1 ETH
    ...
    /// @dev sends ETH to cyberange team
    function transferETH() isOwner external {
        if (this.balance == 0) throw;
        if (!ethFundDeposit.send(this.balance)) throw;
    }

    /// @dev allocates CRAT tokens to pre-sell address.
